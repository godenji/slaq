...
class MainTest(tdb: TestDB) extends DBTest(tdb) {
  ...
  @Test def test(): Unit = {
    db withSession { implicit ss: Session =>
      ...
      val q4 = for (
        u <- Users;
        o <- Orders if (o.orderID in (for { o2 <- Orders filter (o.userID is _.userID) } yield o2.orderID.max))
          & (o.userID is u.id)
      ...
      val q4b = for (
        u <- Users;
        o <- maxOfPer[Orders.type](Orders, _.orderID, _.userID) if o.userID is u.id
      ) yield u.first ~ o.orderID
