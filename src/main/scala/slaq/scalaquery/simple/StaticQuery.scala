...
abstract class StaticQuery[-P, +R](query: String, rconv: GetResult[R], pconv: SetParameter[P])
  ...
  protected type Self <: StaticQuery[P, R]
  protected def copy(query: String = this.query, pconv: SetParameter[P @uncheckedVariance] = this.pconv): Self
...
class StaticQuery0[R](query: String, rconv: GetResult[R], pconv: SetParameter[Unit])
  ...
  protected type Self = StaticQuery0[R]
  protected def copy(query: String, pconv: SetParameter[Unit]): Self =
...
class StaticQuery1[P1, R](query: String, rconv: GetResult[R], pconv: SetParameter[P1])
  ...
  protected type Self = StaticQuery1[P1, R]
  protected def copy(query: String, pconv: SetParameter[P1]): Self =
