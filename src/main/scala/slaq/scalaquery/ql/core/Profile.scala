...
trait Profile {
  ...
  def buildSelect(query: Query[_, _], nc: NamingContext): (SqlBuilder.Result, ValueLinearizer[_]) =
    createQueryBuilder(query, nc).buildSelect

  def buildUpdate(query: Query[_, _], nc: NamingContext): SqlBuilder.Result = createQueryBuilder(query, nc).buildUpdate

  def buildDelete(query: Query[_, _], nc: NamingContext): SqlBuilder.Result = createQueryBuilder(query, nc).buildDelete

  def buildInsert(cb: Any): String = new InsertBuilder(cb, this).buildInsert
  def buildInsert(cb: Any, q: Query[_, _]): SqlBuilder.Result = new InsertBuilder(cb, this).buildInsert(q)
