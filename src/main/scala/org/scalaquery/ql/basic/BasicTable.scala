...
abstract class AbstractBasicTable[T](_schemaName: Option[String], _tableName: String) 
  ...
  def createFinderBy[P](f: this.type => NamedColumn[P])
  	(implicit profile: BasicProfile, tm: TypeMapper[P]): BasicQueryTemplate[P,T] = {
  	
    import profile.Implicit._
    Params[P](tm).flatMap{p=> 
    	Query(this).filter{t=> 
    		ColumnOps.Is( f(t.asInstanceOf[AbstractBasicTable.this.type] ), p)
    	}
    }(profile)
  }
...
abstract class BasicTable[T](_schemaName: Option[String], _tableName: String) 
