...
object Case {
  ...
  class TypedCase[B : TypeMapper, T : TypeMapper]
  	(clauses: List[WhenNode])
  	extends CaseColumn[Option[B]](clauses, ConstColumn.NULL) {
  	
    def If[C <: Column[_] : 
    	Queryable](cond: C) = new TypedWhen[B,T](cond, this)
    	
    def Else(res: Column[T]): 
    	Column[T] = new TypedCaseWithElse[T](clauses, Node(res))
  }

  class TypedWhen[B : TypeMapper, T : TypeMapper]
  	(cond: Node, parent: TypedCase[B,T]) {
  ...
  class TypedCaseWithElse[T : TypeMapper]
  	(clauses: List[WhenNode], elseClause: Node)
